...
public class UserDbms {
	...
	public static ArrayList<String> getPwFind(String name,String tel){
		readMemberData();
		readTeacherData();
		readAdminData();
		...
		for(int i=0; i<memberAllList.size(); i++) {
			...
			&& temp.equals(tel) ) {
				pwList.add(memberAllList.get(i).getPassword()) ;
			}
		}
		...
		for(int i=0; i<teacherAllList.size(); i++) {
			...
			&& temp.equals(tel) ) {
				pwList.add(teacherAllList.get(i).getPassword());
			}
		}
		...
		for(int i=0; i<adminAllList.size(); i++) {
			...
			&& temp.equals(tel) ) {
				pwList.add(adminAllList.get(i).getPassword());
			}
		}
		return pwList;
	}
	...
	public static ArrayList<String> getIdFind(String name,String birth){
		...
		for(int i=0; i<memberAllList.size(); i++) {
			if(memberAllList.get(i).getName().equals(name) 
			&& memberAllList.get(i).getBirth().equals(birth) ) {
				idList.add(memberAllList.get(i).getId()) ;
			}
		}
		for(int i=0; i<teacherAllList.size(); i++) {
			if(teacherAllList.get(i).getName().equals(name) 
			&& teacherAllList.get(i).getBirth().equals(birth) ) {
		...
		for(int i=0; i<adminAllList.size(); i++) {
			if(adminAllList.get(i).getName().equals(name) 
			&& adminAllList.get(i).getBirth().equals(birth) ) {
	...
	public static DataMember searchMemberByIdPw(String id, String pw) {
		...
		for (int i = 0; i < memberAllList.size(); i++) {
			if (memberAllList.get(i).getId().equals(id) 
					&& memberAllList.get(i).getPassword().equals(pw)) {
			...
			public static DataAdmin searchAdminByIdPw(String id, String pw) {
				...
				for (int i = 0; i < adminAllList.size(); i++) {
					if (adminAllList.get(i).getId().equals(id) 
							&& adminAllList.get(i).getPassword().equals(pw)) {
		...
		if(f.exists()) {
			try {
				...
				while ((line = reader.readLine()) != null) {
					...
					memberAllList.add(m);
					if( Integer.parseInt(m.getMemberCode().substring(1)) >= 
						Integer.parseInt(maxMemberCode.substring(1)) ) {
						maxMemberCode = "N"+ String.format("%03d", (Integer.parseInt(m.getMemberCode().substring(1)) +1));
					}
		...
		if(f.exists()) {
			try {
				...
				while ((line = reader.readLine()) != null) {
					...
					if( Integer.parseInt(t.getTeacherCode().substring(1)) >= 
						Integer.parseInt(maxTeacherCode.substring(1)) ) {
						maxTeacherCode = "T"+ String.format("%03d", (Integer.parseInt(t.getTeacherCode().substring(1)) +1));
		...
		if(f.exists()) {
			try {
				...
				while ((line = reader.readLine()) != null) {
					...
					adminAllList.add(a);
					if( Integer.parseInt(a.getAdminCode().substring(1)) >= 
							Integer.parseInt(maxAdminCode.substring(1)) ) {
							maxAdminCode = "A"+ String.format("%03d", (Integer.parseInt(a.getAdminCode().substring(1)) +1));
