...
//						AdminUserView.printUserSearch();
				...
				curTeacher = AdminUserService.getMemberObjectById(inputId); // 변경된 데이터 리로드
				...
				// 탈퇴하지 않은 회원인 경우
				AdminUserService.deleteMember(curTeacher);
	...
	private static void modifyTeacher(DataTeacher t) {
		...
		while (true) {
			...
			if (sel.equals("0")) {
				...
				boolean isInValid = modifyMemberTel(t, scan);
				if (isInValid) {
					continue;
				}
				break;
